...
- **侧栏**：「📄 全部笔记」置顶、「📁 笔记本」可折叠分组（子项缩进）、「🗑 回收站」独立入口、底部固定操作区（新建笔记本 / 标签 / 设置）与同步状态区；顶部「×」蓝色按钮收起侧栏，收起后顶栏「☰」重新展开
- **笔记本**：树形组织、新建、删除（默认把其中笔记移到根目录，弹窗可勾选「同时删除笔记本内所有笔记」级联删除）
- **笔记**：创建 / 编辑 / 删除 / 移动到其他笔记本（长按笔记 →「移动到笔记本」），Markdown 正文
- **标签**：管理、附加到笔记、按标签筛选。侧栏底部「🏷 标签」入口进入标签管理页；管理页支持**新建 / 长按重命名 / 长按删除**（删除走确认弹窗，重命名/新建走输入弹窗），进入页面或同步后自动刷新
- **全文搜索**：标题 + 正文
...
- **回收站**：侧栏「🗑 回收站」虚拟入口（显示已删笔记数）；同步 `deleted_time`，「全部笔记」与搜索自动隐藏已删笔记
- **同步**：与 Joplin Server 双向同步（delta 增量游标 + 冲突时间戳 + 删除上报 `batch_delete` + 网络失败自动重试）；侧栏底部同步区**一键同步 + 实时进度 / 结果显示**（与设置页共用同一份同步逻辑）
...
└── pages/
    ├── Index.ets                 # 主页：侧栏（全部笔记/可折叠笔记本/回收站/操作区/同步区）+ 笔记列表
...
- **侧边栏布局重构**：「📄 全部笔记」置顶为独立项；「📁 笔记本」改为可折叠分组（默认展开，子项缩进）；「🗑 回收站」独立行；标签列表移出侧栏，改为底部「🏷 标签」入口；底部固定操作区（新建笔记本 / 标签 / 设置）；顶部「×」蓝色按钮收起侧栏，收起后顶栏「☰」展开。
- **侧栏同步功能区**：底部同步区一键「⟳」触发同步（蓝色按钮，同步中禁用），第一行标题 + 第二行状态详情（实时进度 / 上次结果），未配置服务器时提示引导去设置。
- **同步逻辑收敛**：`AppStore.runSync()` 统一同步执行逻辑（进度写入 `syncing/syncMessage`，成功自动 `refreshAll`，永不 reject），侧栏与设置页共用，消除重复代码。
- **踩坑**：`Scroll` 设置 `layoutWeight(1)` 后**内容不满一屏时默认垂直居中**，导致侧栏顶部出现大片空白 → `.align(Alignment.Top)` 强制顶部对齐（改 padding 无效）。
